import 'package:sortingalgorithms/constants.dart';
...
class ChartWidget extends StatelessWidget {
  ...
  BarChartGroupData makeGroupDatasmall(
      int x,
      int y, {
        Color barColor = Colors.white,
        double width = 2,
  ...
  List<BarChartGroupData> showingGroups() {
    return numbers.map((f) {
      return nombreTri <= 20 ? makeGroupData(numbers.indexOf(f), f,
          barColor: activeElements.contains(numbers.indexOf(f))
              ? Colors.yellow
              : Colors.white) : makeGroupDatasmall(numbers.indexOf(f), f,
          barColor: activeElements.contains(numbers.indexOf(f))
              ? Colors.yellow
              : Colors.white);
    }).toList();
